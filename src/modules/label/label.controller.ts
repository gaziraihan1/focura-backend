...
export const getLabels = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { workspaceId } = labelsQuerySchema.parse(req.query);

    const labels = await LabelQuery.getLabels(userId, { workspaceId });
...
export const getPopularLabels = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { workspaceId, limit } = popularLabelsQuerySchema.parse(req.query);

    const labels = await LabelQuery.getPopularLabels(userId, { workspaceId, limit });
    res.json({ success: true, data: labels });
  } catch (error) {
    handleError(error, res);
  }
