...
export async function getPersonalQuotaInfo(userId: string): Promise<PersonalQuotaInfo> {
...
export async function getWorkspaceQuotaInfo(
  ...
  const isAdmin     = requesterMembership?.role === "OWNER" || requesterMembership?.role === "ADMIN";
